...
export default function HangoutsPage() {
  ...
  const [activeHangout, setActiveHangout] = useState(null);
  ...
  useEffect(() => {
    const fetchHangoutsAndInvites = async () => {
      try {
        const [hangoutsRes, invitesRes] = await Promise.all([
          API.post("/hangouts/my-hangouts"),
          API.get("/hangouts/invitations"),
        ]);

        setHangouts(hangoutsRes.data.hangouts || []);
        console.log("hangoutsRes", hangoutsRes);
        setInvitations(invitesRes.data.invitations || []);
        console.log("inviteRes", invitesRes);
      } catch (err) {
        toast.error(
          err.response?.data?.message || "Failed to fetch hangouts/invitations."
        );
      } finally {
        setLoading(false);
      }
    ...
    fetchHangoutsAndInvites();
  }, []);

  const handleInvitationResponse = async (hangoutId, status) => {
    try {
      await API.post("/hangouts/invite/respond", { hangoutId, status });
      toast.success(`Invitation ${status} successfully!`);

      // Update UI: Remove the responded invitation
      setInvitations((prev) => prev.filter((inv) => inv.id !== hangoutId));

      // If accepted, fetch updated hangouts
      if (status === "accepted") {
        const hangoutsRes = await API.post("/hangouts/my-hangouts");
        setHangouts(hangoutsRes.data.hangouts || []);
      }
    } catch (err) {
      toast.error(
  ...
  const handleDeleteHangout = async () => {
    try {
      await API.post("/hangouts/delete", { hangoutId: selectedHangoutId });
      toast.success("Hangout deleted successfully!");

      setHangouts((prev) =>
        prev.filter((hangout) => hangout._id !== selectedHangoutId)
      );
    } catch (err) {
      toast.error(
        err.response?.data?.message || "Failed to delete the hangout."
      );
    } finally {
      setIsDeleteDialogOpen(false);
      setSelectedHangoutId(null);
    }
  };

  const openDeleteDialog = (hangoutId) => {
    setSelectedHangoutId(hangoutId);
  ...
  return (
    <div className="flex justify-center min-h-svh bg-gradient-to-br from-zinc-100 to-zinc-300 dark:from-zinc-900 dark:to-zinc-800 p-6 md:p-10">
      ...
      {/* Participants Dialog */}
      <Dialog
        open={dialogType === "participants"}
        onOpenChange={(open) => !open && setDialogType(null)}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>👥 Participants in {activeHangout?.title}</DialogTitle>
          </DialogHeader>

          <div className="space-y-3 max-h-[60vh] overflow-y-auto mt-2">
            {activeHangout?.participants?.map((p) => (
              <div
                key={p.userId}
      ...
      {/* Invitations Dialog */}
      <Dialog
        open={dialogType === "invitations"}
        onOpenChange={(open) => !open && setDialogType(null)}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>📨 Invitations for {activeHangout?.title}</DialogTitle>
          </DialogHeader>

          <div className="space-y-3 max-h-[60vh] overflow-y-auto mt-2">
            {activeHangout?.invitations?.map((inv) => (
              <div
                key={inv.userId}
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>➕ Add Expense for {activeHangout?.title}</DialogTitle>
          </DialogHeader>
          <form
            className="space-y-4"
            onSubmit={async (e) => {
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              📋 Hangout Details:{" "}
              <span className="text-primary">{activeHangout?.title}</span>
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-6 text-sm text-zinc-700 dark:text-zinc-300 max-h-[60vh] overflow-y-auto">
            ...
            <div>
              ...
              {activeHangout?.expenses?.length === 0 ? (
                <p className="text-muted-foreground italic">
                  No expenses added yet.
                </p>
              ) : (
                <div className="space-y-3">
                  {activeHangout?.expenses.map((exp) => (
                    <div
                      key={exp._id}
                    ...
                    >
                      ...
                      <div className="text-xs text-muted-foreground">
                        <span className="block">
                          🧾 Paid by:{" "}
                          <strong>
                            {activeHangout.participants.find(
                              (p) => p.userId === exp.paidBy
                            )?.name || "Unknown"}
                          </strong>
                        </span>
                        <span className="block">
                  ...
                  >
                    ...
                    <div className="text-sm text-right space-y-0.5">
                      <div className="text-green-600 dark:text-green-400">
                        Contributed: ₹{p.contributed.toFixed(2)}
                      </div>
      ...
      <div className="w-full max-w-4xl flex flex-col gap-10">
        ...
        >
          ...
          >
            <Link to="/track/hangouts/new">
              <Plus className="w-5 h-5 mr-2" /> Add Hangout
            </Link>
          </Button>
        </motion.div>
        ...
        <section className="grid gap-6">
          {loading ? (
            ...
            <div className="text-center text-zinc-500 dark:text-zinc-400 italic">
              No hangouts found. Hit that <strong>Add Hangout</strong> button!
            </div>
          ) : (
            <motion.div
                ...
                >
                  <Card className="shadow-md hover:shadow-xl border border-amber-200 dark:border-amber-700 transition-shadow">
                    ...
                    <CardContent className="text-sm space-y-2 text-zinc-600 dark:text-zinc-300">
                      <p>🗓️ {new Date(inv.date).toLocaleDateString()}</p>
                      ...
                      <div className="flex gap-2 mt-3">
                        <Button
                          ...
                          className="flex-1"
                          onClick={() =>
                            handleInvitationResponse(inv.id, "accepted")
                          }
                        >
                          Accept
                        </Button>
                        <Button
                          ...
                          className="flex-1"
                          onClick={() =>
                            handleInvitationResponse(inv.id, "rejected")
                          }
                        >
                          Reject
                        </Button>
